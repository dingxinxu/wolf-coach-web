...
- `worker/src/index.test.js`（33）：AES-GCM 加解密往返 + hashAccessCode 稳定性 + generateAccessCode 字符集 + maskKey 边界 + resolveLLMConfig 优先级 + requireAccessCode 全分支（无码/无效/过期/超限/通过/跨天重置）+ requireAdmin 失败限流（5 次锁 10 分钟）+ MAX_INPUT_CHARS 阈值
