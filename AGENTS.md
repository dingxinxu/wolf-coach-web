...
- **访问码用量计数的原子性 bug（defer）**：`bumpUsage`（成功调 LLM 后才计数，`index.js` 的 `ctx.waitUntil(bumpUsage...)`）仍是 read-modify-write——`validateAccessCode` 读到 `entry.usage.count=N` 后通过闭包传给 `bumpUsage`，期间 LLM 流式调用可能耗时 10s+，并发请求都读到 N，各自 `+1` 写回 `N+1`，**丢失自增**（N 个并发只计 1 次），dailyLimit 实际可被绕过。半公开场景作"防白嫖"够用；真要防并发刷量需 Durable Object 原子计数。修法见 `index.js` `bumpUsage` 注释。
- **admin / verify-code IP 限流非原子（defer）**：`requireAdmin` 的 `admin_fails:{ip}` 和 `handleVerifyCode` 的 `verify_count:{ip}` 都是 read-modify-write，高并发下可绕过（多个请求同时读到旧 count）。KV 最终一致性放大此问题。**NAT/CGNAT 后多用户共享同一 `CF-Connecting-IP`**，正常用户互相挤占配额（某用户输错 5 次 admin 密码会锁掉整个 NAT 后所有 admin 10 分钟）。半公开场景作"业余字典爆破防御"够用；真要防并发爆破需 Durable Object 原子计数或 CF Turnstile。
